    ...
    function mapProductsToItems(rooms) {
        return rooms.map(room => (
            <Card raised key={room._id} >
                <Image href={`/room?_id=${room.roomID}`} size="large" circular src={room.mediaUrl} style={{ maxHeight: 200 }} />
                <Card.Content>
                    <Card.Header>{room.roomName}</Card.Header>
                </Card.Content>
                <Card.Content extra>
                    <Button basic color='grey' onClick={() => { navigator.clipboard.writeText(room.roomID) }}>
                        Copy Room Code
                    </Button>
                </Card.Content>
